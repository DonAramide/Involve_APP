...
class ReportGenerator {
  ...
  static Future<pw.Document> buildInventoryPDF({
    ...
    final headers = [
      isSchool ? 'Fee' : 'Product',
      isSchool ? 'Amount' : 'Price',
      if (!isSchool) 'Stock',
      'Sold',
      'Revenue'
    ];
    final data = reportData.map((item) {
      final stockQty = item['stockQty'] as num;
      return [
        item['name'].toString(),
        CurrencyFormatter.formatWithSymbol(item['price'], symbol: settings.currency),
        if (!isSchool) (stockQty >= 999999 ? 'N/A' : stockQty.toString()),
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.Center(
              child: pw.Text(
                isSchool ? 'FEE REVENUE REPORT' : 'INVENTORY STATUS REPORT',
                style: pw.TextStyle(
                  fontSize: 22, 
                  fontWeight: pw.FontWeight.bold, 
                  color: _getPrimaryColor(settings)
            ...
            pw.Table.fromTextArray(
              ...
              columnWidths: {
                ...
                if (!isSchool) 2: const pw.FlexColumnWidth(1.2),
  ...
  static List<PrintCommand> buildInventoryThermalCommands({
    ...
    final isSchool = settings.businessMode == 'school';
    commands.add(TextCommand(isSchool ? 'FEE REVENUE REPORT' : 'INVENTORY REPORT', isBold: true, align: 'center'));
    if (dateRange != null) {
    ...
    // Table Header
    // Column specs: ITEM(10), QTY(5), SOLD(7), REV(10) = 32 chars
    if (isSchool) {
      commands.add(TextCommand('FEE           SOLD       REV', isBold: true));
    } else {
    ...
    for (final item in reportData) {
      ...
      final qtyNum = item['stockQty'] as num;
      final qty = qtyNum >= 999999 ? '-' : qtyNum.toString();
      final sold = item['totalSold'].toString();
      final rev = CurrencyFormatter.format(item['totalRevenue']).split('.')[0]; 

      if (isSchool) {
        // FEE(14), SOLD(8), REV(10) = 32 chars
        final nameTrunc = name.length > 14 ? name.substring(0, 14) : name.padRight(14);
        final soldPad = sold.padLeft(8);
        final revPad = rev.padLeft(10);
        commands.add(TextCommand('$nameTrunc$soldPad$revPad'));
      } else {
        // ITEM(10), QTY(5), SOLD(7), REV(10) = 32 chars
        final nameTrunc = name.length > 10 ? name.substring(0, 10) : name.padRight(10);
