...
class _SuperAdminSettingsPageState extends State<SuperAdminSettingsPage> {
        ...
        child: Column(
          ...
          children: [
            ...
            _buildTextField(
              ...
              label: 'Device Suffix (e.g. 7A2B9C)',
              icon: Icons.devices,
              maxLength: 6,
            ),
  ...
  Future<void> _generateLicense() async {
    ...
    setState(() {
      _generatedCode =
          'Use Admin Device Activation (server-signed). Client-side license minting is disabled for security.';
    });

    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'License codes must be generated by the Invify backend. Open Admin → Device Activation.',
        ),
      ),
    );
  }
