  ...
  <q-page class="bg-[#0b0f12] text-[#e1e7ec] q-pa-md column op-gap-16">
    ...
    <div class="row items-center justify-between border-bottom q-pb-sm">
      <div class="row items-center op-gap-8">
        ...
        <div>
          <div class="text-operator-title text-white text-weight-bold row items-center op-gap-4">
            <span>AI Operational Intelligence & Predictive Analytics</span>
            <q-badge color="blue-grey-9" text-color="amber-3" class="text-metric-sm">
      ...
      <div class="bg-[#12161a] border-muted q-pa-xs rounded-borders row items-center op-gap-12 no-wrap text-caption">
        <div class="column items-end q-px-xs">
          <span class="text-grey-5" style="font-size: 9px;">Global Reproducibility SLA</span>
          <span class="text-metric-mono text-green-4 text-weight-bold">99.4% PASSED</span>
        </div>
        <q-btn dense flat size="xs" color="cyan-3" label="RUN REPLAY PIPELINE" @click="executeReplayValidation" class="bg-[#172033] q-px-xs text-metric-sm" />
      </div>
    </div>

    <!-- CORE MODEL RELIABILITY GOVERNANCE STRIP (FINAL REFINEMENT #1) -->
    <div class="enterprise-panel bg-[#101826] q-pa-sm rounded-borders column op-gap-8">
      <div class="row items-center justify-between text-caption border-bottom q-pb-xs">
        <span class="text-operator-title text-cyan-3">Model Reliability Governance Grid (Prediction Drift Detection)</span>
        <span class="text-metric-mono text-grey-5" style="font-size: 10px;">Continuously Calibrated against Telemetry Ingest Layers</span>
      </div>
      ...
      <div class="row items-stretch op-gap-8 full-width overflow-x-auto">
        ...
        >
          ...
          <div class="column op-gap-2 q-pt-xs">
            <div class="row items-center justify-between text-grey-4" style="font-size: 9px;">
              <span>Accuracy Decay</span>
              <span class="text-metric-mono" :class="metric.accuracyDecayRate > 0.05 ? 'text-red-3' : 'text-grey-5'">{{ (metric.accuracyDecayRate * 100).toFixed(2) }}%</span>
            </div>
            <div class="row items-center justify-between text-grey-4" style="font-size: 9px;">
              <span>Drift Index</span>
              <span class="text-metric-mono text-amber-3">{{ metric.driftIndex.toFixed(3) }}</span>
            </div>
            <div class="row items-center justify-between text-grey-4" style="font-size: 9px;">
              <span>Avg Confidence</span>
              <span class="text-metric-mono text-cyan-3">{{ (metric.averageConfidence * 100).toFixed(1) }}%</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="row items-start op-gap-16 full-width">
      ...
      <div class="col-12 col-md-7 column op-gap-16">
        ...
        <div class="panel-card bg-[#12161a] border-muted rounded-borders column full-width">
          <div class="panel-header bg-[#161b20] q-px-sm q-py-xs border-bottom row items-center justify-between">
            <div class="row items-center op-gap-4 no-wrap">
              ...
              <span class="text-operator-title text-white text-weight-bold">Correlated RCA Timeline Reconstruction</span>
            </div>
          ...
          <div class="panel-body q-pa-sm column op-gap-8" v-if="activeRcaTrace">
            <div class="text-caption text-grey-4 border-bottom q-pb-xs">
              <span class="text-weight-bold text-white">Probable Causal Chains Identified:</span>
            </div>
            ...
            <div class="column op-gap-4 q-pl-xs border-left-focus">
              ...
              >
                <q-badge color="blue-grey-9" text-color="cyan-3" class="text-metric-sm">{{ idx + 1 }}</q-badge>
            ...
            <div class="bg-[#172033] q-pa-xs rounded-borders q-mt-xs column op-gap-2">
              <span class="text-metric-mono text-amber-3 text-weight-bold" style="font-size: 10px;">ATTRIBUTION ROOT CAUSE:</span>
              <ul class="q-pl-md q-my-none text-grey-3" style="font-size: 11px;">
        ...
        <div class="panel-card bg-[#12161a] border-muted rounded-borders column full-width">
          <div class="panel-header bg-[#161b20] q-px-sm q-py-xs border-bottom row items-center justify-between">
            <div class="row items-center op-gap-4 no-wrap">
              <q-icon name="query_stats" size="xs" color="cyan-3" />
              <span class="text-operator-title text-white text-weight-bold">Predictive Incident Intelligence</span>
            </div>
            <span class="text-metric-mono text-cyan-4" style="font-size: 10px;">Temporal Horizons Grounded</span>
          </div>

          <div class="panel-body q-pa-sm column op-gap-8" v-if="incidentForecast">
            <div class="row items-center justify-between bg-[#0e1215] q-pa-xs rounded-borders border-muted">
              <div class="column">
                <span class="text-grey-5" style="font-size: 9px;">Prediction Horizon</span>
                <span class="text-metric-mono text-amber-3 text-weight-bold">{{ incidentForecast.predictionHorizon }}</span>
              </div>
              <div class="column items-end">
                <span class="text-grey-5" style="font-size: 9px;">Telemetry Ingest Base</span>
                <span class="text-metric-mono text-white">{{ incidentForecast.telemetryEvidence.latencyMs }}ms Roundtrip</span>
              </div>
            </div>

            <div class="column op-gap-2">
              <span class="text-caption text-white text-weight-bold" style="font-size: 11px;">Accelerated Escalation Indicators:</span>
            ...
            <div class="text-grey-5 text-caption q-pt-xs border-top row items-center justify-between" style="font-size: 10px;">
              <span>Impacted Isolation Boundaries: <strong class="text-white">{{ incidentForecast.impactedTenants.join(', ') }}</strong></span>
      ...
      <div class="col-12 col-md-5 column op-gap-16">
        ...
        <div class="panel-card bg-[#12161a] border-muted rounded-borders column full-width">
          <div class="panel-header bg-[#161b20] q-px-sm q-py-xs border-bottom row items-center justify-between">
            <div class="row items-center op-gap-4 no-wrap">
              <q-icon name="rule" size="xs" color="amber-4" />
              <span class="text-operator-title text-white text-weight-bold">Guided Interventions (Advisory-Only)</span>
            </div>
          ...
          <div class="panel-body q-pa-sm column op-gap-8" v-if="activeRemediationTree">
            ...
            <div class="bg-[#1c1510] border-left-critical q-pa-xs rounded-borders column op-gap-2">
              <div class="row items-center justify-between no-wrap">
                <span class="text-white text-weight-bold text-caption">{{ activeRemediationTree.actionType.replace(/_/g, ' ') }}</span>
                <span class="text-metric-mono text-amber-3" style="font-size: 10px;">Weight: {{ activeRemediationTree.causalWeighting }}</span>
              </div>
              <p class="text-grey-4 q-my-none" style="font-size: 11px;">
                {{ activeRemediationTree.rollbackRiskJustification }}
              </p>
            </div>

            <!-- Explainability context string breakdown -->
            <div class="column op-gap-2 bg-[#0e1215] q-pa-xs rounded-borders text-caption text-grey-4" style="font-size: 10px;">
              ...
              <div class="row items-center justify-between border-bottom q-pb-xs">
                ...
                <span class="text-metric-mono text-cyan-3">{{ (activeRemediationTree.confidenceContribution * 100).toFixed(1) }}% Consensus</span>
              </div>
              <div class="row items-center justify-between">
                <span>Active Safety Rules</span>
                <span class="text-metric-mono text-grey-5">RECOMMENDATION_ONLY MODE</span>
              </div>
            </div>
        ...
        <div class="panel-card bg-[#12161a] border-muted rounded-borders column full-width">
          <div class="panel-header bg-[#161b20] q-px-sm q-py-xs border-bottom row items-center justify-between">
            <div class="row items-center op-gap-4 no-wrap">
              <q-icon name="rocket_launch" size="xs" color="cyan-3" />
              <span class="text-operator-title text-white text-weight-bold">Rollout Risk & SLA Degradation Forecast</span>
            </div>
            <span class="text-metric-mono text-grey-5" style="font-size: 10px;">Consensus Vectors Grounded</span>
          </div>

          <div class="panel-body q-pa-sm column op-gap-8" v-if="rolloutForecast && slaForecast">
            
            <div class="row items-center justify-between text-caption text-grey-3 border-bottom q-pb-xs">
            ...
            <div class="row items-center justify-between text-caption text-grey-3 border-bottom q-pb-xs">
              ...
              <span class="text-metric-mono text-amber-3">{{ rolloutForecast.integrityRegressionForecasting.toFixed(2) }}</span>
            </div>
            ...
            <div class="row items-center justify-between text-caption text-grey-3 border-bottom q-pb-xs">
              ...
              <span class="text-metric-mono text-cyan-3">{{ (slaForecast.slaBreachProbability * 100).toFixed(1) }}% Jitter</span>
            </div>

            <div class="row items-center justify-between text-caption text-grey-3">
              <span>WebSocket Congestion Status</span>
              <span class="text-metric-mono text-white">{{ slaForecast.predictiveImpactStatus.replace(/_/g, ' ') }}</span>
            </div>

            <div class="bg-[#161b20] q-pa-xs rounded-borders text-center text-metric-sm text-grey-5 q-mt-xs">
