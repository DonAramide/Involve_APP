...
| **TC-MOB-SCH-005** | Parent Directory & Class Filtering | `parent_list_page.dart` | 1. Open PARENTS from dashboard.<br>2. Search parent by name/phone.<br>3. Select child class filter dropdown.<br>4. Tap clear class filter. | • Parents list filters dynamically.<br>• Class filter isolates parents with children in that tier.<br>• Displays total enrolled children, phone, VA badge, and cumulative family debt. |
| **TC-MOB-SCH-006** | Parent Virtual Account & Profile Hub | `parent_profile_page.dart` | 1. Select parent from list.<br>2. Tap "Generate parent virtual account".<br>3. Verify canonical account card & legacy numbers. | • Profile displays total family debt vs parent credit.<br>• Provisions single 10-digit family VA via Quasar Gateway.<br>• Legacy child account numbers remain displayed as active aliases. |
| **TC-MOB-SCH-007** | Parent Fee Funding & Multi-Child Allocation | `parent_profile_page.dart`<br>`parent_payment_allocator.dart` | 1. Tap "Fund / pay outstanding".<br>2. Select method (Cash, POS, School Account).<br>3. Enter amount.<br>4. Submit and verify allocation breakdown. | • Applies funds across children according to configured share mode (Proportional Auto Share, Waterfall, or Exact Match).<br>• Surplus beyond total debt converts to Parent Credit.<br>• Outputs thermal receipt and PDF statement with per-child Before/Allocated/After balances. |
| **TC-MOB-SCH-008** | Parent Credit Wallet & Mapping Dialog | `parent_profile_page.dart`<br>`parent_payment_share_mode_picker.dart` | 1. On parent profile with credit > 0 and debt > 0, tap "Map parent credit to children".<br>2. Enter custom amounts per child.<br>3. Attempt to over-allocate beyond debt or credit.<br>4. Submit valid allocation. | • Over-allocation input is blocked with alert.<br>• Real-time running total shows mapped vs available credit.<br>• Mapping reduces child balance and decrements parent credit with audit record. |
