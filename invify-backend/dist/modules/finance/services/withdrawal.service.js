...
class WithdrawalService {
    async processRejection(withdrawalId, adminId, reason) {
        // 1. Fetch current withdrawal
        const { data: request, error: reqErr } = await supabase_1.supabase
        ...
        if (updateErr)
            throw updateErr;
        await supabase_1.supabase.from('withdrawal_audit_logs').insert({
    ...
    async processClawback(commissionEventId, adminId, reason) {
        // 1. Fetch Event
        ...
        const { data: adj, error: adjErr } = await supabase_1.supabase
            ...
            .single();
        if (adjErr) {
            if (adjErr.code === '23505')
                throw new Error('Double-clawback prevented');
            throw adjErr;
        }
        // 3. Debit Ledger
        await supabase_1.supabase.from('wallet_ledger').insert({
        ...
        await supabase_1.supabase.from('commission_events')
