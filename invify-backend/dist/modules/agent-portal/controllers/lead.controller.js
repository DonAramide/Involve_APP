...
const integration_engine_service_1 = require("../../../services/integration-engine.service");
class LeadController {
    static async create(req, res) {
        try {
            ...
            const l = await lead_service_1.leadService.createLead(req.body, agent.id, req.ip || '', req.headers['user-agent'] || '');
    ...
    static async list(req, res) {
        try {
            ...
            if (!agent)
                return res.status(404).json({ success: false, message: 'Agent not found' });
            const leads = await lead_service_1.leadService.getLeadsByAgent(agent.id);
    ...
    static async listAll(req, res) {
        try {
            const leads = await lead_service_1.leadService.getAllLeads();
            res.status(200).json({ success: true, data: leads });
        }
        catch (err) {
            res.status(500).json({ success: false, message: err.message });
    ...
    static async convert(req, res) {
        try {
            ...
            // 1. Create a tenant
            const { data: tenant, error: tenantErr } = await supabase_1.supabase
                .from('tenants')
                .insert({
            ...
            const { error: walletErr } = await supabase_1.supabase
                .from('wallets')
                .insert({ tenant_id: tenant.id, balance: 0 });
            if (walletErr)
                console.error('[LeadConvert] Wallet creation error:', walletErr.message);
            ...
            const { data: agentTenant, error: agentTenantErr } = await supabase_1.supabase
                ...
                .single();
            if (agentTenantErr)
                throw agentTenantErr;
            // 4. Create activation progress record
            const { error: progressErr } = await supabase_1.supabase
            ...
            if (progressErr)
                console.error('[LeadConvert] Progress record creation error:', progressErr.message);
            // 5. Update lead status and link in agent_leads
            const { error: updateLeadErr } = await supabase_1.supabase
                .from('agent_leads')
                .update({
                status: 'converted',
                ...
                .eq('id', id);
            if (updateLeadErr)
                console.error('[LeadConvert] Lead update error:', updateLeadErr.message);
            try {
                await integration_engine_service_1.integrationEngine.publish('LEAD_CONVERTED', 'LEAD_MANAGEMENT', id, {
                    agentId: agent.id,
                    tenantId: tenant.id,
                    leadId: id
                });
            ...
            try {
                ...
                await agentRepository.logAudit(agent.id, 'LEAD', id, 'CONVERT', lead.status, { converted_tenant_id: tenant.id, status: 'converted' }, req.ip || '', req.headers['user-agent'] || '');
            }
            catch (auditErr) {
