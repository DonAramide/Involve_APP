...
class AgentRepository {
     ...
     */
    async createAgent(agentData, profileData) {
        // Insert Agent
        ...
        if (profileError) {
            // Manual rollback
            await supabase_1.supabase.from('agents').delete().eq('id', agent.id);
            throw profileError;
        }
        return agent;
    }
     ...
     */
    async updateStatus(id, newStatus, oldStatus, changedBy, reason) {
        // 1. Log History
     ...
     */
    async logAudit(actorId, entityType, entityId, action, oldValue, newValue, ipAddress, userAgent) {
