...
exports.ApkVaultService = void 0;
// src/services/apk-vault.service.ts
const supabase_1 = require("../db/supabase");
class ApkVaultService {
    ...
    static async addApk(apkData, operatorEmail = 'system') {
        // Enforce 3 slots max
    ...
    static async updateApkSlot(slotId, apkData, operatorEmail = 'system') {
        const { data: existing, error: findError } = await supabase_1.supabaseAdmin
            .from('apk_vault')
            .select('*')
            .eq('id', slotId)
            .maybeSingle();
        ...
        if (!dist.find((v) => v.version === apkData.version)) {
    ...
    static async updateApkUrl(slotId, s3Url, operatorEmail = 'system') {
        const { data, error } = await supabase_1.supabaseAdmin
            .from('apk_vault')
            .update({
            s3_url: s3Url,
    ...
    static async logDeployment(logData, operatorEmail = 'system') {
        ...
        if (apkId) {
            const { data: apk, error: fetchErr } = await supabase_1.supabaseAdmin
                .from('apk_vault')
                .select('*')
                .eq('id', apkId)
                .maybeSingle();
            if (!fetchErr && apk) {
                if (action === 'INSTALL') {
                    ...
                    const verDist = dist.find((v) => v.version === targetVersion);
                    if (verDist) {
...
exports.ApkVaultService = ApkVaultService;
//# sourceMappingURL=apk-vault.service.js.map