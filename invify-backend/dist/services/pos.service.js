...
const terminal_audit_service_1 = require("./terminal-audit.service");
...
class PosService {
    ...
    static get ENCRYPTION_KEY() {
        if (!process.env.POS_ENCRYPTION_KEY) {
            throw new Error('[POS Service] POS_ENCRYPTION_KEY environment variable is required. ' +
     ...
     */
    static async saveConfig(updatedBy = 'system') {
        try {
            ...
            const { error } = await supabase_1.supabaseAdmin.from('pos_routing_configs').insert({
    ...
    static tenantCategoryCache = new Map();
    static cacheTenantCategory = async () => { };
    // ─── Terminal Parameters Cache ─────────────────────────────────────────────
    static kimonoParamsCache = new Map();
    static CACHE_TTL_MS = 12 * 60 * 60 * 1000; // 12 hours
    ...
    static async updateRoutingConfig(newConfig, adminId = 'Admin', reason = 'Updated POS routing configuration') {
        ...
        if (newConfig.hosts) {
            for (const newHost of newConfig.hosts) {
                ...
                if (oldHost) {
                    if (oldHost.authToken && !newHost.authToken)
    ...
    static async getTransactionHistory(_tenantId) {
    ...
    static async getObservabilityMetrics() {
        ...
        const hostSuccessRate = {};
        const hostAvgLatency = {};
        const hostFailoverCount = {};
        const baseLatencies = {
            express_pay: 140,
            kimono: 250,
            medusa: 190,
        ...
        for (const h of this.routingConfig.hosts) {
            ...
            hostFailoverCount[code] = this.transactionHistory.filter(t => t.host?.toUpperCase() === codeUpper && t.status === 'Declined' && (t.statusCode === '96' || t.statusCode === '91')).length;
        }
        const auditRes = await terminal_audit_service_1.TerminalAuditService.getAuditLog({ limit: 20 });
        return {
            totalTransactions,
    ...
    static async processTransaction(params) {
        const pan = params.emvData?.pan ?? params.emvData?.cardNo ?? '';
        ...
        if (process.env.OFFLINE_LOCAL_AUTH !== 'true' && params.tenantId && params.tenantId !== 'default') {
            supabase_1.supabaseAdmin.from('pos_transaction_attempts').insert([{
                    id: pendingId,
        ...
        try {
            ...
            if (fallback) {
                ...
                try {
                    ...
                    response = this.buildErrorResponse(route.name, '96', 'System Error — all hosts unavailable');
                }
            }
            else {
                response = this.buildErrorResponse(route.name, '96', err.message || 'System Error');
            }
        }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    static async fetchKimonoParams(terminalId, amountKobo, binCode) {
        ...
        if (!kimonoHost)
            throw new Error('[Kimono] Host configuration not found');
        ...
        try {
            const params = await this.httpGet(url, this.buildCpointHeaders(terminalId));
            if (params.code !== '00') {
    ...
    static clearKimonoParamsCache(terminalId) {
        if (terminalId) {
     ...
     */
    static buildKimonoPayload(emvData, terminalParams, pan) {
        ...
        const minorAmount = emvData.amountAuthorisedNumeric?.replace(/^0+/, '') ||
    ...
    static async processViaKimono(params) {
        const terminalId = params.terminalId || params.emvData?.terminalId;
        if (!terminalId)
            throw new Error('[Kimono] terminalId is required');
        ...
        console.log(`[Kimono] ─────────────────────────────────────────────────────────\n`);
        const responseJson = await this.httpPost(url, payload, this.buildCpointHeaders(terminalId));
        ...
        return {
            ...
            maskedPan: this.maskPan(payload.pan),
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    static async processViaTcpSocket(params, route) {
    ...
    static tcpExchange(host, port, packet) {
     ...
     */
    static parseIsoMessage(raw, context = 'ISO8583') {
        try {
     ...
     */
    static extractField39Heuristic(raw) {
        try {
     ...
     */
    static detectCardScheme(pan) {
        if (!pan)
            return 'UNKNOWN';
        const cleanPan = pan.replace(/\s+/g, '');
        if (cleanPan.startsWith('4'))
            return 'VISA';
        if (/^(5[1-5]|222[1-9]|22[3-9]|2[3-6]|27[0-1]|2720)/.test(cleanPan))
            return 'MASTERCARD';
        if (/^(506[0-9]|507[8-9]|6500|6509|9792)/.test(cleanPan))
            return 'VERVE';
        return 'UNKNOWN';
    }
    static getHostSlaScore(hostCode) {
    ...
    static determineRoute(amountNaira, tenantId, transactionType, cardScheme, simulatedHealthOverrides) {
        ...
        const scoredHosts = activeHosts.map(host => {
            ...
            if (host.supportedTenantCategories && host.supportedTenantCategories.length > 0) {
                const matchesCategory = category.toLowerCase() === 'all' || host.supportedTenantCategories.some(c => c.toLowerCase() === category.toLowerCase());
                if (!matchesCategory) {
                    score -= 2000;
                }
            }
            if (host.supportedCardSchemes && host.supportedCardSchemes.length > 0) {
                const matchesScheme = cardScheme.toLowerCase() === 'all' || host.supportedCardSchemes.some(s => s.toLowerCase() === cardScheme.toLowerCase());
                if (!matchesScheme) {
                    score -= 3000;
                }
            }
            if (host.supportedTransactionTypes && host.supportedTransactionTypes.length > 0) {
                const matchesType = transactionType.toLowerCase() === 'all' || host.supportedTransactionTypes.some(t => t.toLowerCase() === transactionType.toLowerCase());
                if (!matchesType) {
                    score -= 3000;
                }
            }
            const profile = this.routingConfig.tenantRoutingProfiles.find(p => p.category.toLowerCase() === category.toLowerCase());
            if (profile) {
    ...
    static getFailover(currentName) {
    ...
    //  HTTP HELPERS
    // ═══════════════════════════════════════════════════════════════════════════
    static buildCpointHeaders(terminalId) {
        return {
    ...
    //  UTILITY HELPERS
    // ═══════════════════════════════════════════════════════════════════════════
    static getUdfValue(params, code) {
        if (!params.udfDataList)
            return '';
    ...
    static buildErrorResponse(host, code, message) {
        return { paymentSuccess: false, statusCode: code, message, host };
    }
    static isoResponseMessage(code) {
        const messages = {
            '00': 'Approved',
    ...
    //  TRANSACTION LOGGER
    // ═══════════════════════════════════════════════════════════════════════════
    static async updateTransaction(id, params, response) {
        ...
        entry.rawResponse = JSON.stringify(response.kimonoResponse ||
            response.isoFields ||
        ...
        if (process.env.OFFLINE_LOCAL_AUTH !== 'true' && entry.tenantId && entry.tenantId !== 'default' && entry.tenantId !== 'Unknown') {
            supabase_1.supabaseAdmin.from('pos_transaction_attempts').update({
                status: entry.status,
                status_code: entry.statusCode,
                host: entry.host,
                masked_pan: entry.maskedPan,
...
PosService.tenantCategoryCache = new Map();
PosService.cacheTenantCategory = async (tenantId) => {
    try {
