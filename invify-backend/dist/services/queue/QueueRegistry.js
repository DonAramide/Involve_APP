...
class QueueRegistry {
    ...
    static async getPendingMessages(queueName) {
        if (this.useInMemory) {
            const now = new Date();
            return this.inMemoryMessages.filter(m => m.queue_name === queueName &&
                m.status === 'PENDING' &&
