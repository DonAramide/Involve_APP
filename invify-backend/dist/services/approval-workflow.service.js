...
class ApprovalWorkflowService {
     ...
     */
    static async approveCommission(ticketId, operatorId) {
        ...
        const { error } = await supabase_1.supabaseAdmin.rpc('process_commission_approval', {
     ...
     */
    static async rejectCommission(ticketId, reason, operatorId) {
        const { data: ticket, error: fetchErr } = await supabase_1.supabaseAdmin
            .from('approval_queue')
            .select('*')
            .eq('id', ticketId)
     ...
     */
    static async executeClawback(agentId, amount, reason, justification, operatorId) {
        const { data, error } = await supabase_1.supabaseAdmin.rpc('execute_commission_clawback', {
            p_agent_id: agentId,
            p_amount: amount,
            p_reason: reason,
     ...
     */
    static async escalateTicket(ticketId, operatorId) {
        const { data: ticket, error: fetchErr } = await supabase_1.supabaseAdmin
            .from('approval_queue')
            .select('*')
            .eq('id', ticketId)
