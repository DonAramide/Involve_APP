...
class CommissionEngineService {
     ...
     */
    static async evaluateAcquisitionReward(agentId, tenantId, merchantCategoryId) {
        try {
            // 1. Deterministically fetch the exact active commission plan responsible for this calculation
            const { data: legacyPlan, error: legacyPlanError } = await supabase_1.supabaseAdmin
                .from('commission_plans')
                .select('id, base_bounty, holding_period_days')
                .eq('event_type', 'ACTIVATION')
                .is('effective_to', null)
                .single();
            if (legacyPlanError || !legacyPlan)
                throw new Error('No valid commission plan found for this calculation');
     ...
     */
    static async calculateRevenueShare(agentId, transactionType, platformNetRevenue, tenantId) {
        try {
            ...
            const { error: ledgerError } = await supabase_1.supabaseAdmin
     ...
     */
    static async checkAndUpgradeTier(agentId) {
        // 1. Count tenants
