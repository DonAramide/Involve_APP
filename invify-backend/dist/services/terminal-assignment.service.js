...
exports.TerminalAssignmentService = void 0;
...
class TerminalAssignmentService {
    static async assign(params) {
        try {
            const { data: terminal, error: termErr } = await supabase_1.supabase
            ...
            if (termErr || !actualTerminal) {
                const { data: byId } = await supabase_1.supabase.from('terminal_inventory').select('*').eq('id', params.terminalId).maybeSingle();
                actualTerminal = byId;
            }
            if (!actualTerminal)
                throw new Error('Bank Terminal ID not found');
            ...
            if (params.deviceId) {
                const { data: existingAssignment } = await supabase_1.supabase
                    .from('terminal_inventory')
                    .select('terminal_id')
                ...
                if (existingAssignment) {
                    throw new Error(`Device already has terminal ${existingAssignment.terminal_id} assigned. Unassign first.`);
                }
            }
            ...
            if (params.hardwareId) {
                const { data: hardware } = await supabase_1.supabase
                    .from('terminal_inventory')
                    .select('*')
                ...
                if (hardware) {
                    ...
                    await supabase_1.supabase.from('terminal_inventory').delete().eq('id', params.hardwareId);
                }
            }
    ...
    static async unassign(params) {
        try {
            ...
            let actualTerminal = terminal;
            if (termErr || !actualTerminal) {
                const { data: byTid } = await supabase_1.supabase.from('terminal_inventory').select('*').eq('terminal_id', params.terminalId).maybeSingle();
    ...
    static async transfer(params) {
        try {
            ...
            const { data: existingForTarget } = await supabase_1.supabase
                .from('terminal_inventory')
                .select('terminal_id')
            ...
            if (existingForTarget) {
                throw new Error(`Target device already has terminal ${existingForTarget.terminal_id} assigned`);
            }
            const { data: updated, error } = await supabase_1.supabase
                .from('terminal_inventory')
                .update({
...
exports.TerminalAssignmentService = TerminalAssignmentService;
//# sourceMappingURL=terminal-assignment.service.js.map