...
const infraHistory = { cpu: [], memory: [] };
class DashboardService {
    static async getOverviewKPIs() {
        const { data, error } = await supabase_1.supabase.from('v_dashboard_kpis').select('*').single();
    ...
    static async getActiveModules() {
        ...
        const { count: txCount } = await supabase_1.supabase.from('ledger_entries').select('*', { count: 'exact', head: true });
        const { count: tenantCount } = await supabase_1.supabase.from('tenants').select('*', { count: 'exact', head: true });
        const { count: casesCount } = await supabase_1.supabase.from('reconciliation_cases').select('*', { count: 'exact', head: true });
    ...
    static async getAlerts() {
        const { data, error } = await supabase_1.supabase.from('v_dashboard_alerts').select('*');
        if (error) {
    ...
    static async getGovernance() {
        const { data, error } = await supabase_1.supabase.from('v_dashboard_governance').select('*');
        if (error) {
    ...
    static async getTenantIntelligence() {
        ...
        const { data, error } = await supabase_1.supabase.from('v_dashboard_tenant_intelligence').select('*');
        if (error) {
    ...
    static async getRecommendations() {
        const { count: cases } = await supabase_1.supabase.from('reconciliation_cases').select('*', { count: 'exact', head: true }).eq('status', 'OPEN');
        const recs = [];
        if (cases && cases > 0) {
