...
class DeviceController {
    ...
    static async getDevices(req, res) {
        try {
            ...
            const { data: devices, error: devError } = await supabase_1.supabase
                .from('devices')
                .select('*')
            ...
            if (tenantIds.length > 0) {
                const { data: tenants, error: tenError } = await supabase_1.supabase
                    .from('tenants')
                    .select('id, name, plan')
                    .in('id', tenantIds);
                if (!tenError && tenants) {
     ...
     */
    static async getActivations(req, res) {
        try {
            // 1. Fetch activations raw data
            const { data: activations, error: actError } = await supabase_1.supabase
     ...
     */
    static async createActivation(req, res) {
        try {
            const user = req.user;
            ...
            try {
                const { data, error } = await supabase_1.supabaseAdmin.from('tenants').select('name').eq('id', tenantId).single();
                if (error)
                    throw error;
                if (data && data.name) {
            ...
            const code = license_util_1.LicenseGenerator.generate(businessName, Number(durationDays) || 30, Number(planIndex) || 0, deviceSuffix || '0');
    ...
    static async validateCode(req, res) {
        try {
            ...
            try {
                const { data: inventoryRecord } = await supabase_1.supabase
            ...
            try {
                const { data: upsertedDevice, error: upsertError } = await supabase_1.supabase
                    .from('devices')
     ...
     */
    static async onboardDevice(req, res) {
        try {
    ...
    static async getDeviceStatus(req, res) {
        try {
            ...
            const { data, error } = await supabase_1.supabase.from('device_status').select('*').eq('device_id', deviceId).single();
            if (error)
                throw error;
            return res.status(200).json(data);
        }
     ...
     */
    static async getDeviceTelemetry(req, res) {
        try {
            const { deviceId } = req.params;
            const { data, error } = await supabase_1.supabase.from('device_telemetry').select('*').eq('device_id', deviceId).order('created_at', { ascending: false }).limit(50);
            if (error)
                throw error;
            return res.status(200).json(data);
        }
     ...
     */
    static async getDeviceAlerts(req, res) {
        try {
            const { deviceId } = req.params;
            const { data, error } = await supabase_1.supabase.from('device_alerts').select('*').eq('device_id', deviceId).order('created_at', { ascending: false });
            if (error)
                throw error;
            return res.status(200).json(data);
        }
     ...
     */
    static async upgradeToCompany(req, res) {
        try {
            const { deviceId } = req.params;
            ...
            const { data: inventoryRecord, error: invError } = await supabase_1.supabase
                .from('terminal_inventory')
                .select('*')
            ...
            const { data: updatedDevice, error: updateError } = await supabase_1.supabase
                .from('devices')
                .update({
                device_category: 'COMPANY_DEVICE',
                device_role: deviceRole,
            ...
            const { error: invUpdateError } = await supabase_1.supabase
                .from('terminal_inventory')
                .update({
