...
class TenantKycController {
    static async uploadKyc(req, res) {
        try {
            const authUserId = req.user?.tenantId || req.user?.id;
            if (!authUserId) {
            ...
            const documentUrl = await s3_service_1.S3Service.uploadFile(file.buffer, file.originalname, file.mimetype, `tenants/kyc/${authUserId}`);
            ...
            const { data: doc, error: insertError } = await supabase_1.supabase.from('tenant_kyc_documents').insert({
                tenant_id: authUserId,
                document_type: type,
                document_url: documentUrl,
                status: 'PENDING'
            ...
            await supabase_1.supabase.from('tenants').update({ kyc_status: 'PENDING' }).eq('id', authUserId);
            return res.status(200).json({
                success: true,
    ...
    static async getKycDocuments(req, res) {
        try {
            ...
            const { data, error } = await supabase_1.supabase.from('tenant_kyc_documents').select('*').eq('tenant_id', tenantId);
            if (error) {
