...
class InvoiceItemRepository {
    static async bulkUpsert(client, items) {
        ...
        items.forEach((item, index) => {
            ...
            values.push(item.id, item.invoiceId, item.itemId, item.quantity, item.unitPrice, item.type || 'product');
            paramIndex += 6;
        });
        const query = `
...
//# sourceMappingURL=invoice-item.repository.js.map