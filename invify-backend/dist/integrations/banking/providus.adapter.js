...
exports.ProvidusBankAdapter = void 0;
...
class ProvidusBankAdapter {
    ...
    async nameEnquiry(params) {
        ...
        const result = {
            accountName: 'PROVIDUS MOCK RECIPIENT',
            isVerified: true
        };
    ...
    async executeTransfer(params) {
        ...
        if (forced === 'TIMEOUT') {
            ...
            return { providerReference: `ref_providus_err_${Date.now()}`, status: 'TIMEOUT' };
        }
        if (forced === 'FAILED') {
            await this.audit('TRANSFER', 'supports_nip_transfer', params, { error: 'Failed' }, 400, params.financialEventId);
            return { providerReference: `ref_providus_err_${Date.now()}`, status: 'FAILED' };
        }
        const ref = `ref_providus_${Date.now()}`;
        const result = {
    ...
    async validateWebhook(payload, signature) {
        if (!signature)
            return false;
        return signature === 'providus_signature_token';
    }
...
exports.ProvidusBankAdapter = ProvidusBankAdapter;
//# sourceMappingURL=providus.adapter.js.map