...
-- Money never moves on maker create. Checker (different operator) approves,
-- then Invify calls Quasar fail-closed and posts the local ledger.
...
-- Append-only per-case audit trail (never UPDATE/DELETE these rows).
