...
export class TransferThroughputBenchmark {
  /**
   ...
   */
  static async run(count = 300): Promise<ThroughputResult> {
    QueueEngine.registerHandler('TRANSFER', async (_payload) => {
    ...
    for (let i = 0; i < count; i++) {
      const id = await QueueEngine.enqueue('TRANSFER', {
