...
export class InsightsService {
   ...
   */
  static async getClassInsights(tenantId: string, classLevel: string) {
    const { data: rawStats, error } = await supabase.rpc('get_class_engagement_stats', {
    ...
    if (trend.length >= 2) {
      ...
      const avgPrevious = trend.slice(0, trend.length - 1).reduce((acc: number, val: any) => acc + val.rate, 0) / (trend.length - 1);
