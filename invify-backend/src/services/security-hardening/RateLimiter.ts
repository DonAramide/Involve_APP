...
export class RateLimiter {
   ...
   */
  static configure(endpoint: string, config: RateLimitConfig) {
  ...
  static check(identifier: string, endpoint: string): RateLimitResult {
    ...
    if (allowed) {
      ...
      entry.blocked = true;
      StructuredLogger.warn(`[RateLimiter] BLOCKED ${identifier} on ${endpoint}`, {
        count,
