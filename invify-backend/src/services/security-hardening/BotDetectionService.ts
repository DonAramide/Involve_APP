...
const KNOWN_BOT_PATTERNS: Array<{ pattern: RegExp; score: number; label: string }> = [
...
const KNOWN_CRAWLER_PATTERNS: Array<{ pattern: RegExp }> = [
...
export class BotDetectionService {
  ...
  static analyzeRequest(req: BotAnalysisRequest): BotAnalysisResult {
    ...
    let classification: BotClassification;
    let action: BotAction;

    if (score >= BLOCK_THRESHOLD) {
      classification = 'CONFIRMED_BOT';
      action = 'BLOCK';
      StructuredLogger.warn(`[BotDetection] BLOCKED score=${score}`, { ua: ua.substring(0, 100), signals });
    } else if (score >= FLAG_THRESHOLD) {
      classification = 'SUSPECTED_BOT';
      action = 'FLAG';
      StructuredLogger.warn(`[BotDetection] FLAGGED score=${score}`, { ua: ua.substring(0, 100), signals });
    } else {
