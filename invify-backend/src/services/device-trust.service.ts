...
export class DeviceTrustService {
  static async verifyDeviceOrThrow(deviceId: string, tenantId: string) {
    ...
    if (!device) {
      ...
      if (!userDevice) throw new Error('DEVICE_NOT_FOUND');
      if (userDevice.status === 'blocked') throw new Error('DEVICE_REVOKED');
      if (userDevice.status === 'pending') throw new Error('DEVICE_SUSPENDED');
    ...
    const tenant: any = device.license;
