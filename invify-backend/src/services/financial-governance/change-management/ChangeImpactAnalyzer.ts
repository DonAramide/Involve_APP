...
const CASCADE_MAP: Record<PolicyType, PolicyType[]> = {
...
const IMPACT_LEVEL_MAP: Record<PolicyType, ImpactLevel> = {
  TREASURY:        'CRITICAL',
  LIQUIDITY:       'HIGH',
  SETTLEMENT:      'HIGH',
  ROUTING:         'HIGH',
...
export class ChangeImpactAnalyzer {
  static analyze(policyType: PolicyType, proposedData: Record<string, any>): ImpactAnalysis {
    ...
    const cascadingDomains: DomainImpact[] = cascades.map((type) => ({
      domain: type,
  ...
  private static getCapabilities(type: PolicyType): string[] {
