...
export class BudgetEnforcementService {
   ...
   */
  static async checkBudget(budgetId: string, requiredAmount: number): Promise<boolean> {
   ...
   */
  static async consumeBudget(budgetId: string, amount: number): Promise<boolean> {
    // Assuming we create an RPC 'consume_commission_budget' in Phase 1 SQL
    const { data, error } = await supabaseAdmin.rpc('consume_commission_budget', {
