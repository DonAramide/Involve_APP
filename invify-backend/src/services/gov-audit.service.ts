...
function operatorNameFromEmail(email?: string | null): string {
  const local = String(email || '').split('@')[0];
  if (!local) return 'Unknown';
  return local.replace(/[-_.]/g, ' ').toUpperCase();
}

/** Historical + live activation-code generation (who issued the key). */
function normalizeDeviceActivation(raw: any): AuditEntry {
  const createdBy = raw.created_by || raw.createdBy || 'unknown';
  return {
    id: `dact-${raw.id || Date.now()}`,
    timestamp: raw.created_at || new Date().toISOString(),
    module: 'DEVICE',
    action: 'GENERATE_ACTIVATION_CODE',
    user_email: createdBy,
    user_name: operatorNameFromEmail(createdBy),
    ip_address: '127.0.0.1',
    location: 'Unknown',
    target: raw.activation_code || raw.device_id || '-',
    status: 'success',
    tenant_id: raw.tenant_id || null,
    metadata: {
      ...
      device_suffix: raw.device_suffix,
...
function activationGenerateKey(entry: AuditEntry): string | null {
  if (entry.action !== 'GENERATE_ACTIVATION_CODE') return null;
  const target = String(entry.target || '').trim().toUpperCase();
  if (!target || target === '-') return null;
  return `${target}|${entry.tenant_id || ''}`;
}

/** Prefer live audit_logs (IP/location) over the device_activations projection. */
function preferActivationAudit(existing: AuditEntry, incoming: AuditEntry): AuditEntry {
  const existingLive = !String(existing.id).startsWith('dact-');
  const incomingLive = !String(incoming.id).startsWith('dact-');
  if (incomingLive && !existingLive) return incoming;
  if (existingLive && !incomingLive) return existing;
  const existingIp = existing.ip_address && existing.ip_address !== '127.0.0.1';
  const incomingIp = incoming.ip_address && incoming.ip_address !== '127.0.0.1';
  if (incomingIp && !existingIp) return incoming;
  return existing;
}
...
export class GovAuditService {
  ...
  static async getLedger(filters: {
    ...
    // 6. Activation keys generated by operators (who issued the code)
    try {
      let actQuery = supabaseAdmin
        ...
        .limit(1000);
      if (tenantFilter && isUuid(tenantFilter)) {
        actQuery = actQuery.eq('tenant_id', tenantFilter);
      }
      const { data: activations } = await actQuery;
      if (activations?.length) {
        allLogs.push(...activations.map(normalizeDeviceActivation));
      }
    } catch { /* optional source */ }
    ...
    // Collapse GENERATE_ACTIVATION_CODE from both audit_logs and device_activations
    const activationByKey = new Map<string, AuditEntry>();
    const withoutActivationDupes: AuditEntry[] = [];
    for (const entry of allLogs) {
      const actKey = activationGenerateKey(entry);
      if (!actKey) {
        withoutActivationDupes.push(entry);
        continue;
      }
      const existing = activationByKey.get(actKey);
      activationByKey.set(actKey, existing ? preferActivationAudit(existing, entry) : entry);
    }
    allLogs = [...withoutActivationDupes, ...activationByKey.values()];
