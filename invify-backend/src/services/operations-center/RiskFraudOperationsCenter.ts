export interface RiskContext {
  ...
  ipAddress: string;
  country: string;
  velocityWindowCount: number; // Transaction frequency in last 60 seconds
...
export class RiskFraudOperationsCenter {
  private static blockedAccounts: Set<string> = new Set();
  private static suspiciousMerchants: Map<string, { riskRating: number; chargebackRatio: number }> = new Map();
  private static blacklistedNames: Set<string> = new Set(['EVIL ORG', 'SCAMMER INC']);
  private static trustedDevices: Set<string> = new Set(['dev-fingerprint-001', 'dev-fingerprint-002']);
  private static geoBlacklist: Set<string> = new Set(['KP', 'SY', 'IR']);
  ...
  static blockAccount(accountId: string) {
    this.blockedAccounts.add(accountId);
  }

  static flagMerchant(merchantId: string, riskRating: number, chargebackRatio: number) {
