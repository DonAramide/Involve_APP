  ...
  static async getAdminAnalytics(req: Request, res: Response) {
    try {
      ...
      const [
        ...
        // 1. Activation & Growth
        supabase.from('tenants').select('id, name, created_at, onboarded_at, plan'),
        
        // 2. Usage (Last 30 Days)
        supabase.from('ai_usage').select('id, tenant_id, source, created_at, response_time_ms').gte('created_at', last30Days),
        
        // 3. Invites Funnel
        supabase.from('invites').select('id, status'),
      ...
      const planDistrib = activeSubs.reduce((acc: any, s: any) => {
