...
export class CommissionController {
  ...
  static async approveCommission(req: Request, res: Response) {
    try {
      ...
      const { id } = req.params;
      const operatorId = (req as any).user?.id || 'sys_admin_root';

      const success = await ApprovalWorkflowService.approveCommission(id, operatorId);
      if (!success) {
  ...
  static async rejectCommission(req: Request, res: Response) {
    try {
      ...
      const success = await ApprovalWorkflowService.rejectCommission(id, reason, operatorId);
      if (!success) {
  ...
  static async executeClawback(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.status(503).json({ success: false, error: 'Database Connection Unavailable. Write actions are disabled in offline mode.' });
      }
      ...
      const success = await ApprovalWorkflowService.executeClawback(agentId, amount, reason, justification || '', operatorId);
      if (!success) {
  ...
  static async listAuditHistory(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.json({
          success: true,
      ...
      const { data: clawbacks, error: clawbacksErr } = await supabaseAdmin
  ...
  static async listAgentProgress(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
  ...
  static async listPlansAndTargets(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.json({
          success: true,
      ...
      const { data: categoryRules, error: catErr } = await supabaseAdmin
        .from('merchant_category_commission_rules')
        .select('*, merchant_categories(name)');

      const { data: performanceRules, error: perfErr } = await supabaseAdmin
        .from('performance_target_rules')
        .select('*');

      const { data: terminalRules, error: termErr } = await supabaseAdmin
        .from('terminal_target_rules')
        .select('*');

      const { data: categories, error: mcErr } = await supabaseAdmin
      ...
      const formattedPrograms = (programs || []).map((prog: any) => {
        const versions = (prog.commission_plan_versions || []).map((ver: any) => {
  ...
  static async deleteProgram(req: Request, res: Response) {
    try {
      ...
      const { error } = await supabaseAdmin.from('commission_programs').delete().eq('id', id);
      if (error) throw error;
  ...
  static async createVersion(req: Request, res: Response) {
    try {
      ...
      const { data: programRule, error: ruleErr } = await supabaseAdmin
  ...
  static async cloneVersion(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.status(503).json({ success: false, error: 'Database offline. Write actions disabled.' });
      }
      ...
      const { data: srcVer, error: srcVerErr } = await supabaseAdmin
        .from('commission_plan_versions')
        .select('*')
        .eq('id', sourceVersionId)
        .single();
      if (srcVerErr || !srcVer) throw new Error('Source plan version not found');

      const { data: srcRule, error: srcRuleErr } = await supabaseAdmin
        .from('commission_program_rules')
        .select('*')
        .eq('plan_version_id', sourceVersionId)
        .single();
      if (srcRuleErr || !srcRule) throw new Error('Source program rules not found');

      const { data: newVer, error: newVerErr } = await supabaseAdmin
        .from('commission_plan_versions')
        .insert({
      ...
      const { data: newRule, error: newRuleErr } = await supabaseAdmin
        .from('commission_program_rules')
        .insert({
  ...
  static async activateVersion(req: Request, res: Response) {
    try {
      ...
      const { data: ver, error: verErr } = await supabaseAdmin
        ...
        .eq('id', id)
        .single();
      if (verErr || !ver) throw new Error('Plan version not found');
  ...
  static async updateVersionRules(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.status(503).json({ success: false, error: 'Database offline. Write actions disabled.' });
      }
  ...
  static async deleteVersion(req: Request, res: Response) {
    try {
      ...
      const { error } = await supabaseAdmin.from('commission_plan_versions').delete().eq('id', id);
      if (error) throw error;
  ...
  static async deleteCategoryRule(req: Request, res: Response) {
    try {
      ...
      const { error } = await supabaseAdmin.from('merchant_category_commission_rules').delete().eq('id', id);
      if (error) throw error;
  ...
  static async createPerformanceRule(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.status(503).json({ success: false, error: 'Database offline. Write actions disabled.' });
      }
  ...
  static async updatePerformanceRule(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.status(503).json({ success: false, error: 'Database offline. Write actions disabled.' });
      }
  ...
  static async deletePerformanceRule(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.status(503).json({ success: false, error: 'Database offline. Write actions disabled.' });
      }
      ...
      const { error } = await supabaseAdmin.from('performance_target_rules').delete().eq('id', id);
      if (error) throw error;
  ...
  static async createTerminalRule(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.status(503).json({ success: false, error: 'Database offline. Write actions disabled.' });
      }
  ...
  static async updateTerminalRule(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.status(503).json({ success: false, error: 'Database offline. Write actions disabled.' });
      }
  ...
  static async deleteTerminalRule(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.status(503).json({ success: false, error: 'Database offline. Write actions disabled.' });
      }
      ...
      const { error } = await supabaseAdmin.from('terminal_target_rules').delete().eq('id', id);
      if (error) throw error;
  ...
  static async listCampaignsAndBudgets(req: Request, res: Response) {
    try {
      if (isOfflineMode()) {
        return res.json({ success: true, budgets: MOCK_BUDGETS, campaigns: MOCK_CAMPAIGNS, degradedMode: true });
      }

      const { data: budgets, error: budErr } = await supabaseAdmin.from('commission_budgets').select('*');
      const { data: campaigns, error: campErr } = await supabaseAdmin.from('commission_campaigns').select('*');

      if (budErr || campErr) throw new Error('Failed to query campaigns/budgets');
  ...
  // 9. Simulator
  static async simulateCommission(req: Request, res: Response) {
    try {
      ...
      if (!dryRun) {
        ...
        if (eventType === 'FULLY_ACTIVATED' && tenantId) {
          const ok = await CommissionEngineService.evaluateAcquisitionReward(agentId, tenantId, merchantCategoryId || '');
          await IncentiveEngineService.evaluateTargets(agentId, 'FULLY_ACTIVATED', { tenantId });
        ...
        if (eventType === 'FIRST_TRANSACTION' && platformNetRevenue) {
          const ok = await CommissionEngineService.calculateRevenueShare(agentId, 'CARD', platformNetRevenue, tenantId);
          await IncentiveEngineService.evaluateTargets(agentId, 'FIRST_TRANSACTION', { platformNetRevenue });
