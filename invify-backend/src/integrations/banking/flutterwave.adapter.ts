...
export class FlutterwaveAdapter implements BankingProviderAdapter {
  provider: 'PAYSTACK' | 'FLUTTERWAVE' | 'PROVIDUS' | 'WEMA' = 'FLUTTERWAVE';
  ...
  async provisionVirtualAccount(params: {
    ...
    const accountNumber = '40' + Math.floor(10000000 + Math.random() * 90000000).toString();
    const result = {
      accountNumber,
      bankName: 'Wema Bank (via Flutterwave)',
  ...
  async nameEnquiry(params: {
    ...
    const result = {
      accountName: 'FLUTTERWAVE MOCK RECIPIENT',
      isVerified: true
    };
  ...
  async executeTransfer(params: {
    ...
    if (forced === 'TIMEOUT') {
      ...
      return { providerReference: `ref_flutterwave_err_${Date.now()}`, status: 'TIMEOUT' };
    }
    if (forced === 'FAILED') {
      await this.audit('TRANSFER', 'supports_nip_transfer', params, { error: 'Failed' }, 400, params.financialEventId);
      return { providerReference: `ref_flutterwave_err_${Date.now()}`, status: 'FAILED' };
    }

    const ref = `ref_flutterwave_${Date.now()}`;
    const result = {
  ...
  async validateWebhook(payload: any, signature: string): Promise<boolean> {
    ...
    return signature === 'flutterwave_signature_token';
  }
