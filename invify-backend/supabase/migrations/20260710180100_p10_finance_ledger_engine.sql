/*
=============================================================================
Migration: p10_finance_ledger_engine
Description: Creates the double-entry accounting ledger system and wallets.

--- MIGRATION GOVERNANCE ---
Rollback Strategy:
DROP FUNCTION IF EXISTS public.request_payout_with_lock;
DROP FUNCTION IF EXISTS public.process_ledger_double_entry;
...
SELECT * FROM information_schema.tables WHERE table_name IN ('ledgers', 'ledger_entries', 'wallets');
SELECT proname FROM pg_proc WHERE proname IN ('prevent_ledger_modification', 'process_ledger_double_entry', 'request_payout_with_lock');

Backwards Compatibility Notes:
Fully backwards compatible. Additive structure. Existing financial APIs will need to be routed to these functions.

Deployment Notes:
Critical path. Must be deployed before any transaction settlement workflows are activated.

Risk Assessment:
Medium Risk. Introduces constraints and triggers that strictly enforce immutability.
=============================================================================
*/
